...
export const MigrationManager = () => {
  ...
  const [existingMigrations, setExistingMigrations] = useState<ExistingMigration[]>([]);
  ...
  useEffect(() => {
    ...
    supabase.rpc("get_migration_history").then(({ data, error }) => {
      if (error || !Array.isArray(data)) return;
      setExistingMigrations(
        data.map((entry: any) => ({
          version: String(entry.executed_at || ""),
          name: String(entry.name || ""),
          statements_count: Number(entry.statements_count || 0),
        })),
      );
    });
  }, []);
          ...
          <CollapsibleContent className="mt-3 space-y-2">
            ...
            {existingMigrations.length === 0 && (
              <p className="py-2 text-center text-xs text-muted-foreground">
                אין היסטוריה זמינה לחשבון המנהל המחובר
              </p>
            )}
            {existingMigrations.map((m) => (
              <div
                key={m.version}
