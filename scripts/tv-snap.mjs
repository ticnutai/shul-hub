...
const rows = await (await fetch(`${url}/rest/v1/tv_snapshots?select=device_id,taken_at,image`, { headers })).json();
if (!Array.isArray(rows) || rows.length === 0) { console.log("no stored screenshots"); process.exit(0); }
for (const r of rows) {
  const when = new Date(r.taken_at).toLocaleString("he-IL", { timeZone: "Asia/Jerusalem" });
